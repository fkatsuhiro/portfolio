...
const ContributionCard = ({ title, url, repoName, date }: { title: string; url: string; repoName: string; date: string }) => (
  <a 
    href={url} 
    target="_blank" 
    ...
    const subTabs = [
        {
            ...
            techStack.some(tech => item.repository.name.toLowerCase().includes(tech.repo.toLowerCase()))
            ).length 
        },
        { 
            id: 'issues', 
      ...
      {activeTab === 'product' && (
        <div className="animate-in fade-in duration-500 grid grid-cols-1 md:grid-cols-2 gap-6">
          {products.map((product, index) => (
            <div key={index} className="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {product.tech.map(t => (
                  <span key={t} className="text-xs font-mono text-slate-400 bg-slate-50 dark:bg-slate-900 px-2 py-1 rounded border border-slate-100 dark:border-slate-800">{t}</span>
                ))}
              </div>
              <a href={product.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-sm font-medium hover:underline">View Project →</a>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="animate-in fade-in duration-500">
            ...
            {subTabs.map((tab) => (
              <button
                ...
                className={`pb-4 text-sm font-bold transition-all relative ${
                  subTab === tab.id ? 'text-slate-900 dark:text-white' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
                {tab.label}
          ...
          <div className="space-y-16">
            ...
            {contributions[subTab].length === 0 && (
              <div className="py-20 text-center text-slate-400">No {subTab} found.</div>
            )}
          </div>
        </div>
      )}
